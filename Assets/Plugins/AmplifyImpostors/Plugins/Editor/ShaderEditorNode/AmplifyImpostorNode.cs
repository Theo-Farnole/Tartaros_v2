...
namespace AmplifyShaderEditor
{
	...
	public sealed class AmplifyImpostorNode : ParentNode
	{
		...
		private void GenerateSphereImpostorVertex( bool isSRP = false )
		{
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float4 fractions = 1 / float4( sizeX, _AI_FramesY, sizeY, UVscale );" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float3 objectCameraPosition = mul( ai_WorldToObject, float4( worldCameraPos, 1 ) ).xyz - _AI_Offset.xyz; " );
		...
		private void GenerateSphereImpostorFragment( ref MasterNodeDataCollector dataCollector, bool withSpec = true, bool withEmission = true, bool isSRP = false )
		{
			...
			IOUtils.AddFunctionLine( ref m_functionBody,
			"float4 albedoSample = "+ GeneratorUtils.GenerateSamplingCall( ref dataCollector, WirePortDataType.SAMPLER2D, "_Albedo","sampler_Albedo", "frameUV.xy",MipType.MipBias, "_AI_TextureBias")+";" );

			IOUtils.AddFunctionLine( ref m_functionBody, "o.Alpha = ( albedoSample.a - _AI_Clip );" );
			...
			if( withSpec )
			{
				...
				IOUtils.AddFunctionLine( ref m_functionBody, 
				"float4 specularSample = " + GeneratorUtils.GenerateSamplingCall(ref dataCollector,WirePortDataType.SAMPLER2D,"_Specular","sampler_Specular", "frameUV.xy",MipType.MipBias,"_AI_TextureBias" )+";" );
			...
			if( withEmission )
			{
				...
				IOUtils.AddFunctionLine( ref m_functionBody, 
				"float4 emissionSample = "+ GeneratorUtils.GenerateSamplingCall( ref dataCollector, WirePortDataType.SAMPLER2D, "_Emission","sampler_Emission","frameUV.xy",MipType.MipBias,"_AI_TextureBias")+";" );
			...
			for( int i = 0; i < m_extraSamplers; i++ )
			{
				if( GetInputPortByArrayId( i ).IsConnected && GetOutputPortByArrayId( i ).IsConnected )
				{
					...
					IOUtils.AddFunctionLine( ref m_functionBody, 
					"output" + i + " = "+ GeneratorUtils.GenerateSamplingCall( ref dataCollector, WirePortDataType.SAMPLER2D, m_extraPropertyNames[ i ], "sampler_"+ m_extraPropertyNames[ i ],"frameUV.xy", MipType.MipBias, "_AI_TextureBias") +";" );
				}
			}
			//IOUtils.AddFunctionLine( ref m_functionBody, "float4 normalSample = tex2Dbias( _Normals, float4( frameUV.xy, 0, _AI_TextureBias) );" );
			IOUtils.AddFunctionLine( ref m_functionBody,
			"float4 normalSample = "+ GeneratorUtils.GenerateSamplingCall( ref dataCollector, WirePortDataType.SAMPLER2D, "_Normals","sampler_Normals", "frameUV.xy",MipType.MipBias, "_AI_TextureBias") +";" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float depth = remapNormal.a * _AI_DepthSize * 0.5 * length( ai_ObjectToWorld[ 2 ].xyz );" );
			if( !isSRP )
			{
		...
		private void GenerateImpostorVertex( bool isSRP = false )
		{
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "vertex.xyz = billboard + _AI_Offset.xyz;" );
			IOUtils.AddFunctionLine( ref m_functionBody, "normal.xyz = objectCameraDirection;" );
		...
		void GenerateImpostorFragment( ref MasterNodeDataCollector dataCollector, bool withSpec = true, bool withEmission = true, bool isSRP = false )
		{
			...
			if( m_forceClampFrames )
			{
				...
				IOUtils.AddFunctionLine( ref m_functionBody, "float2 step_1 = ( parallax1 - octaFrame.zw ) * _AI_Frames;" );
				IOUtils.AddFunctionLine( ref m_functionBody, "float4 step23 = ( float4( parallax2, parallax3 ) -  octaFrame.zwzw ) * _AI_Frames - cornerDifference;" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float depthOffset = ( ( parallaxSample1.a * weights.x + parallaxSample2.a * weights.y + parallaxSample3.a * weights.z ) - 0.5 /** 2.0 - 1.0*/ ) /** 0.5*/ * _AI_DepthSize * length( ai_ObjectToWorld[ 2 ].xyz );" );
			if( !isSRP )
			{
				IOUtils.AddFunctionLine( ref m_functionBody, "#if defined(SHADOWS_DEPTH)" );
				IOUtils.AddFunctionLine( ref m_functionBody, "if( unity_LightShadowBias.y == 1.0 ) " );
