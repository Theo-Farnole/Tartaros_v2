...
struct Input
{
	...
	UNITY_FOG_COORDS(5)
	float2 worldSpacePosition : TEXCOORD6;
};
...
void SplatmapVert(inout appdata_full v, out Input data)
{
	...
	data.worldSpacePosition = mul(unity_ObjectToWorld, v.vertex).xz;
...
void SplatmapMix(Input IN, half4 defaultAlpha, out half4 splat_control, out half weight, out fixed4 mixedDiffuse, inout fixed3 mixedNormal, float2 intersectionUVs)
{
	...
	mixedDiffuse += splat_control.r * tex2D(_Splat0, intersectionUVs) * half4(1.0, 1.0, 1.0, defaultAlpha.r);
...
#ifdef _TERRAIN_NORMAL_MAP
	fixed4 nrm = 0.0f;
	nrm += splat_control.r * tex2D(_Normal0, intersectionUVs);
